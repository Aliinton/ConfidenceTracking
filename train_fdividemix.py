...
def get_args():
    ...
    parser.add_argument('--alg', type=str, default='f-DivideMix')
    parser.add_argument('--arc', type=str, default='resnet50')
    parser.add_argument('--seed', type=int, default=1)
    parser.add_argument('--save_dir', type=str, default='save/f-DivideMix')
...
def eval_train(epoch, args, net, eval_loader, sample_selector, loader):
    ...
    features = []
    labels = []
    for img, label, idx in tqdm(eval_loader):
        img = img.to(args.device)
        labels.append(label)
        if args.arc == 'resnet50':
            feature = net.maxpool(net.relu(net.bn1(net.conv1(img))))
            feature = net.avgpool(net.layer4(net.layer3(net.layer2(net.layer1(feature)))))
            feature = feature.view(feature.size(0), -1)
            pred = net.fc(feature).softmax(-1)
            sample_selector.update(idx, pred, epoch)
            features.append(feature.cpu())
        elif args.arc == 'InceptionResNetV2':
            feature = net.features(img)
            feature = net.avgpool_1a(feature)
            feature = feature.view(feature.size(0), -1)
            pred = net.last_linear(feature).softmax(-1)
            sample_selector.update(idx, pred, epoch)
            features.append(feature.cpu())
        elif args.arc == 'vgg19':
            feature = net.features(img)
            feature = net.avgpool(feature)
            feature = torch.flatten(feature, 1)
            for i, layer in enumerate(list(net.classifier)):
                if i < 6:
                    feature = layer(feature)
                else:
                    pred = layer(feature).softmax(-1)
            assert feature.size(1) == 4096, feature.shape
            assert pred.size(1) == args.num_classes, pred.shape
            features.append(feature.cpu())
            sample_selector.update(idx, pred, epoch)
        else:
            logging.info(f'{args.arc} is not supported')
    ...
    if (epoch + 1) >= args.warmup_epoch:
        ...
        probs = torch.ones(args.n_samples)

    if args.ct_refine:
        res = sample_selector.select_clean_labels(epoch).cpu()
        add_mask = res & probs.lt(args.p_threshold)
...
if __name__ == '__main__':
    ...
    for epoch in range(args.n_epoch):
        ...
        if epoch < args.warmup_epoch:
            ...
            # Train Net1
            labeled_loader, unlabeled_loader = loader.run('ssl', clean_mask=clean_mask2)
            train(args, net1, net2, optimizer1, scaler, labeled_loader, unlabeled_loader, clean_prob2[clean_mask2])
            scheduler1.step()

            # Train Net2
            labeled_loader, unlabeled_loader = loader.run('ssl', clean_mask=clean_mask1)
            train(args, net2, net1, optimizer2, scaler, labeled_loader, unlabeled_loader, clean_prob1[clean_mask1])
            scheduler2.step()
        ...
        clean_prob1 = eval_train(epoch, args, net1, eval_train_loader, sample_selector1, loader)
        clean_prob2 = eval_train(epoch, args, net2, eval_train_loader, sample_selector2, loader)
