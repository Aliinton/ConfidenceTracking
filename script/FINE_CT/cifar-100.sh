...
SAVE_DIR="save/FINE+CT/alpha${ALPHA}/"
for SEED in 1 2 3 4 5
do
  python train.py --experiment_name $EXPERIMENT_NAME --dataset cifar-100 --seed $SEED \
  --noise_type $NOISE_TYPE --noise_rate $NOISE_RATE --n_epoch 150 --alg FINE \
  ...
  --warmup_epoch $WARMUP --selector 'FINE+CT' --data_dir $DATA_DIR --save_dir $SAVE_DIR --alpha $ALPHA
done
