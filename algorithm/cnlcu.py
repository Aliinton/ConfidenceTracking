...
class CNLCU:
    def __init__(self, args):
        ...
        self.mean_loss1 = torch.zeros(len(self.noisy_label)).to(args.device)
        self.mean_loss2 = torch.zeros(len(self.noisy_label)).to(args.device)
        self.select_times1 = torch.ones(len(self.noisy_label)).to(args.device)
        self.select_times2 = torch.ones(len(self.noisy_label)).to(args.device)
    ...
    def _update_co_lambda(self, epoch):
        self.co_lambda = 1e-4 * max(0.0, 1 - epoch / self.args.milestones[0])
        logging.info(f'sigma^2: {self.co_lambda}')

    def _reinitialize(self):
        self.mean_loss1 = torch.zeros(len(self.noisy_label)).to(self.device)
        self.mean_loss2 = torch.zeros(len(self.noisy_label)).to(self.device)
        self.select_times1 = torch.ones(len(self.noisy_label)).to(self.device)
        self.select_times2 = torch.ones(len(self.noisy_label)).to(self.device)
    ...
    def soft_process(self, x):
        return torch.log(1+x+x*x/2)

    def _cal_loss(self, logit1, logit2, label, index):
        batch_size = len(label)
        # get "soft loss"
        soft_loss1 = self.soft_process(F.cross_entropy(logit1, label, reduction='none'))
        soft_loss2 = self.soft_process(F.cross_entropy(logit2, label, reduction='none'))

        # update mean loss
        self.mean_loss1[index] = (self.epoch * self.mean_loss1[index] + soft_loss1) / (self.epoch + 1)
        self.mean_loss2[index] = (self.epoch * self.mean_loss2[index] + soft_loss2) / (self.epoch + 1)

        # add confidence bound bias to loss
        t = self.epoch + 1
        delta = self.co_lambda
        nt1 = self.select_times1[index]
        nt2 = self.select_times2[index]
        confidence_bound1 = delta * (t + delta * math.log(2 * t) / (t ** 2)) / (nt1 + 1 - delta)
        confidence_bound2 = delta * (t + delta * math.log(2 * t) / (t ** 2)) / (nt2 + 1 - delta)
        corrected_loss1 = self.mean_loss1[index] - confidence_bound1
        corrected_loss2 = self.mean_loss2[index] - confidence_bound2

        # co-teaching loss
        n = int((1 - self.drop_rate) * batch_size)
        idx1 = corrected_loss1.argsort()[:n]
        idx2 = corrected_loss2.argsort()[:n]
        ...
        # update select times
        self.select_times1[idx1] += 1
        self.select_times2[idx2] += 1

        # save sample selection result
